class_name LocalizationHelper extends Node
...
const SETTINGS_NAME := "localization"
...
static func set_setting(setting: String, value: Variant) -> void:
	...
	if not ProjectSettings.has_setting(path):
		push_error("Failed to update setting \"%s\": the setting does not exist in the Project Settings. Verify the configuration in Project > Project Settings > General > Godot Easy > Translation." % setting)
